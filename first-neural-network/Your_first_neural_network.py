...
#             print(error[None,:])
            ...
            #hidden_error = output_error_term* delta_weights_h_o
            
            
            output_error_term = np.array([error])
           
            temp = (hidden_outputs*(1-hidden_outputs))[:,None]
            hidden_error_term = hidden_error * temp
            
            

            # Weight step (input to hidden)
            delta_weights_i_h += np.dot(X[:,None], hidden_error_term.T)
            #output_error_term * hidden_outputs
            # Weight step (hidden to output)
            delta_weights_h_o += np.dot(hidden_outputs[:,None], output_error_term.T)
